...
wp_enqueue_script('employer-single');
?>
<div class="employer-detail-v2 iwj-single-parallax">
    ...
    if ($cover_image) {
        ...
        $cover_image_url = IWJ_PLUGIN_URL . '/assets/img/employer-parallax-default.jpg';
    }
    ?>
                ...
                <div class="employer-info-top" itemscope itemtype="http://schema.org/Organization">
                    <div class="info-top">
                        ...
                        <div class="conttent-right">
                            <h3 class="title theme-color" itemprop="name"><?php echo $employer->get_title(); ?></h3>
                    ...
                    <div class="follow-button">
                        <?php if ( is_user_logged_in() ) { ?>
                            ...
                            if ( current_user_can( 'apply_job' ) ) {
                                ...
                                $followed_text = $followed ? __( '<i class="ion-android-send"></i> Followed', 'iwjob' ) : __( '<i class="icon-injob-bell"></i> Follow Us', 'iwjob' );
                                ?>
                            ...
                            <?php } ?>
                        <?php } else { ?>
                            <a href="#" class="action-button follow iwj-follow iwj-btn" data-toggle="modal" data-target="#iwj-login-popup"><span data-hover="<?php echo __( 'Follow Us', 'iwjob' ); ?>"><i class="icon-injob-bell"></i><?php echo __( 'Follow Us', 'iwjob' ); ?></span></a>
                        <?php } ?>
                        <?php
        ...
        <div class="container">
            <div class="row">
                ...
                if ( ! $show_employer_public_profile || ( $show_employer_public_profile && is_user_logged_in() ) ) { ?>
                    ...
                    $employer_video = $employer->get_video();
                    if ( $gallery || $employer_video ) {
                        wp_enqueue_script( 'bxslider' );
                        wp_enqueue_style( 'bxslider' );
                        ?>
                        <div class="iwj-gallery-detail">
                            <ul class="bxslider">
                                <?php if ( $employer_video ) {
                                    $iframe_video_string = wp_oembed_get( $employer_video[0]);
                                    preg_match('/src="([^"]+)"/', $iframe_video_string, $match_video);
                                    $url_video = $match_video[1];

                                    $video_poster_url = '';
                                    $video_poster = $employer->get_video_poster();
                                    if ($video_poster) {
                                    ...
                                    ?>
                                    <li>
                                        <div class="iwj-employer-detail-video" data-embed="<?php echo esc_attr($url_video); ?>" style="background-image: url('<?php echo esc_url($video_poster_url); ?>')">
                                ...
                                if ( $gallery ) {
                                    foreach ( $gallery as $image ) {
                                        ...
                                        $image_url = inwave_resize( $image_url, 770, 435, true );
                                        if ( $image_url ) {
                                            echo '
                                            <li>
                    ...
                    <div class="employer-detail-info">
                        <?php if ( $description = $employer->get_description( true ) ) : ?>
                            <div class="iwj-employerdl-des">
                                <div class="title"><h3 class="theme-color"><?php echo __( 'Company Detail', 'iwjob' ); ?></h3></div>
                    ...
                    if ( ! $disable_review ) {
                        ...
                        if ( ( $employer_review && count( $employer_review['result'] ) ) || ( is_user_logged_in() && current_user_can( 'apply_job' ) ) ) {
                            ...
                            <div class="title"><h3 class="theme-color"><?php echo __( 'Company Review:', 'iwjob' ); ?></h3>
                            </div>
                    ...
                    if ( $open_positions ) { ?>
                        <div class="iwj-open-position" id="iwj-open-position">
                            <h3 class="iw-title-border"><?php echo count($open_positions); echo __( ' Classes are opening', 'iwjob' ); ?></h3>
                            <div class="iwj-jobs">
                                <?php
                                    ...
                                    <div class="job-item <?php echo $job->is_featured() ? 'featured-item' : ''; ?>">
                                        <div class="job-info">
                                            ...
                                            <div class="info-company">
                                                <ul>
                                                    <?php if ( $categories = $job->get_categories() ) {
                    ...
                    <?php if ( $employer_sidebar && is_active_sidebar( 'sidebar-employer-v2' ) ) : ?>
                        <div class="iwj-sidebar-sticky <?php echo esc_attr( inwave_get_classes( 'sidebar', $employer_sidebar ) ) ?>">
                            <div class="widget-area" role="complementary">
                                <?php dynamic_sidebar( 'sidebar-employer-v2' ); ?>
                            </div>
                        </div>
                    <?php endif;
                } else {
