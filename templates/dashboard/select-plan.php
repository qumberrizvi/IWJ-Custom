...
$new_job        = isset( $_GET['new-job'] ) ? (bool) $_GET['new-job'] : false;
...
if ( $notice ) {
	if ( $notice == '1' ) {
		$message = __( 'Your membership plan has expired. Please renew it or select a new package to continuing', 'iwjob' );
	} else {
		$message = __( 'You have reached the limit of renewal listings. Please renew it or select a new package to continuing', 'iwjob' );
	}
	?>
...
<div class="iwj-user-packages iwj-main-block">
	<div class="iwj-plans-table iwj-table-overflow-x">
		<form action="" method="POST">
				...
				<div class="owl-carousel" data-plugin-options="<?php echo htmlspecialchars( json_encode( $data_plugin_options ) ); ?>">
					...
					if ( $packages_query->have_posts() ) {
						...
						while ( $packages_query->have_posts() ) {
							...
							echo '<div class="choose-package"><button type="submit" class="buy-now" name="plan_id" value="' . $package->get_id() . '">' . esc_html__( "Choose Package", 'iwjob' ) . '</button></div>';
			...
			<input type="hidden" name="is_new_job" value="<?php echo ( isset( $new_job ) && $new_job ) ? $new_job : ''; ?>">
		</form>
