...
class IWJ_Admin_Plan_Package{
    ...
    static function init(){
        self::$fields = array(
            ...
            'stripe_plan_id' => array(
                'name' => __('Stripe Plan ID', 'iwproperty'),
                'desc' => __('If you have created a plan for this package, fill it here, otherwise the system will automatically create it if it does not already exist', 'iwproperty'),
                'id' => IWJ_PREFIX . 'stripe_plan_id',
                'type' => 'text',
            ),
            'number_job' => array(
            ...
            'support_service'=>array(
                'name' => __( 'Support Service', 'iwjob' ),
                'id'   => IWJ_PREFIX.'support_service',
                'type' => 'select',
                'options' => array(
                    '0' => __('No', 'iwjob'),
                    '1' => __('Yes', 'iwjob'),
            ...
            /*'type_package'=>array(
                'name' => __( 'Type Package', 'iwjob' ),
                'id'   => IWJ_PREFIX.'type_package',
                'type' => 'select',
                'options' => array(
                    '' => __('Normal', 'iwjob'),
                    'free' => __('Free', 'iwjob'),
                    'popular' => __('Popular', 'iwjob'),
                ),
            ),*/
        );
    ...
    static function register_metabox(){
        add_meta_box('iwj-employer-meta-box', __('Package Metabox Info', 'iwjob'), array( __CLASS__, 'metabox_html'), 'iwj_plan', 'normal', 'high');
    }
        ...
        <div class="iwj-metabox wp-clearfix">
            <table class="form-table">
                <?php
                    ...
                    echo '<th class="iwjmb-label"><label>'.__( 'Duration', 'iwjob' ).'</label></th>';
                    echo '<td>';
                    $field = IWJMB_Field::call( 'normalize', self::$fields['expiry'] );
                    $meta = IWJMB_Field::call( $field, 'post_meta', $post_id, $saved );
                    echo IWJMB_Field::call( $field, 'input', $meta );
                    $field = IWJMB_Field::call( 'normalize', self::$fields['expiry_unit'] );
                    $meta = IWJMB_Field::call( $field, 'post_meta', $post_id, $saved );
                    echo IWJMB_Field::call( $field, 'input', $meta );
                    echo '</td>';
                    echo '</tr>';
                    ...
                    //job support
                    echo '<tr class="iwjmb-field">';
                    echo '<th class="iwjmb-label"><label>'.__( 'Support Service', 'iwjob' ).'</label></th>';
                    echo '<td>';
                    $field = IWJMB_Field::call( 'normalize', self::$fields['support_service'] );
                    $meta = IWJMB_Field::call( $field, 'post_meta', $post_id, $saved );
                    echo IWJMB_Field::call( $field, 'input', $meta );
                    echo '</td>';
                    echo '</tr>';
                    ...
                    //type support
                    /*echo '<tr class="iwjmb-field">';
                    echo '<th class="iwjmb-label"><label>'.__( 'Type Package', 'iwjob' ).'</label></th>';
                    echo '<td>';
                    $field = IWJMB_Field::call( 'normalize', self::$fields['type_package'] );
                    $meta = IWJMB_Field::call( $field, 'post_meta', $post_id, $saved );
                    echo IWJMB_Field::call( $field, 'input', $meta );
                    echo '</td>';
                    echo '</tr>';*/
    ...
    static function save_post($post_id){
        if(isset($_POST) && $_POST){
            if(get_post_type($post_id) == 'iwj_plan'){
    ...
    static function columns_content( $column, $post_ID ) {
        ...
        if($screen->post_type == 'iwj_plan'){
            ...
            if ($column == 'status') {
                $package = IWJ_Plan::get_package($post_ID);
    ...
    static function status_label($statuses, $post){
        if($post->post_type == 'iwj_plan'){
            if($post->ID == iwj_option('free_plan_id')){
                return array(__('Free', 'iwjob'));
            }
            if($post->ID == iwj_option('featured_plan_id')){
