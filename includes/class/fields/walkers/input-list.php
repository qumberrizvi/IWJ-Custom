 ...
 */
class IWJMB_Walker_Input_List extends IWJMB_Walker_Base {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		$output .= '<ul class="iwjmb-input-list">';
	}
	...
	public function start_el( &$output, $object, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		$attributes = IWJMB_Field::call( 'get_attributes', $this->field, $object->$id );

		$output .= sprintf(
			'<li><label><input %s %s>%s</label>',
			IWJMB_Field::render_attributes( $attributes ),
			checked( in_array( $object->$id, $this->meta ), 1, false ),
			IWJMB_Field::filter( 'choice_label', $object->$label, $this->field, $object )
		);
	}
