...
jQuery( function ( $ ) {
	...
	var views = iwjmb.views = iwjmb.views || {},
		models = iwjmb.models = iwjmb.models || {},
	...
	MediaStatus = views.MediaStatus = Backbone.View.extend( {
		tagName: 'span',
		className: 'iwjmb-media-status',
		template: wp.template( 'iwjmb-media-status' ),
	...
	MediaButton = views.MediaButton = Backbone.View.extend( {
		className: 'iwjmb-add-media button',
		tagName: 'a',
		events: {
			click: function () {
				...
				this._frame = wp.media( {
					className: 'media-frame iwjmb-media-frame',
					multiple: true,
					title: i18niwjmbMedia.select,
					editing: true,
					library: {
		...
		render: function () {
			this.$el.text( i18niwjmbMedia.add );
			return this;
		},
	...
	MediaItem = views.MediaItem = Backbone.View.extend( {
		tagName: 'li',
		className: 'iwjmb-media-item',
		template: wp.template( 'iwjmb-media-item' ),
		initialize: function ( options ) {
			this.controller = options.controller;
			this.render();
		...
		events: {
			...
			'click .iwjmb-remove-media': function ( e ) {
				this.controller.removeItem( this.model );
				return false;
			},

			'click .iwjmb-edit-media': function ( e ) {
				// Destroy the previous collection frame.
				if ( this._frame ) {
					//this.stopListening( this._frame );
					this._frame.dispose();
	...
	function initMediaField() {
		new MediaField( {input: this, el: $( this ).siblings( 'div.iwjmb-media-view' )} );
	}


	$( ':input.iwjmb-file_advanced' ).each( initMediaField );
	$( '.iwjmb-input' )
		.on( 'clone', ':input.iwjmb-file_advanced', initMediaField );
} );
