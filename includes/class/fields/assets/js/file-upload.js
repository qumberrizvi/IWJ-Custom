...
jQuery( function ( $ ) {
	...
	UploadButton = views.UploadButton = Backbone.View.extend( {
		...
		initUploader: function () {
			...
			this.uploader.bind( 'PostInit', function ( up, files ) {
				//fix gallery not working in EDGE
                if(navigator && navigator.userAgent.indexOf( 'Edge' ) !== -1){
					$('.iwjmb-browse-button').on('click', function (e) {
						e.preventDefault();
						$(this).next('.moxie-shim').find('input').trigger('click');
					});
                }
            } );
