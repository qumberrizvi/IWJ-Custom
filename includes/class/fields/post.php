 ...
 * Post field class.
 */
class IWJMB_Post_Field extends IWJMB_Object_Choice_Field {
	...
	public static function normalize( $field ) {
		...
		if ( empty( $field['placeholder'] ) ) {
			$field['placeholder'] = __( 'Select a post', 'iwjob' );
			if ( is_string( $field['query_args']['post_type'] ) && post_type_exists( $field['query_args']['post_type'] ) ) {
				...
				$field['placeholder'] = sprintf( __( 'Select a %s', 'iwjob' ), $post_type_object->labels->singular_name );
			}
		}
	...
	public static function post_meta( $post_id, $saved, $field ) {
		...
		return parent::post_meta( $post_id, $saved, $field );
	}
